...
`LinearMixedModel` 类型代表的是一个线性混合模型。
典型情况下，该类型是从一个公式 `Formula` 和一个合适的数据 `data` 类型，通常是一个数据框 `DataFrame`.
```@docs
LinearMixedModel
```
...
为了方便展示，本包以 `.feather` 形式迁移和存储了 *R* 语言 *lme4* 包中的数据集。
这其中包含数据集`Dyestuff`和`Dyestuff2`。
...
*Julia*与*R*的公式语言类似，均基于(Wilkinson and Rogers [1973](https://dx.doi.org/10.2307/2346786))。在Julia中公式必须包裹在宏`@formula`中。
```@docs
@formula
```
